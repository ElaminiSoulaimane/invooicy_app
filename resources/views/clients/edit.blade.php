<x-app-layout>
    <div class="container mx-auto py-8">
        <h1 class="text-2xl font-bold mb-4">{{ __('messages.edit_client') }}</h1>
        <form action="{{ route('clients.update', $client->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name"
                    class="mt-1 block w-full border-perf-green rounded-[26px] shadow-sm" value="{{ $client->name }}"
                    required>
            </div>
            <div class="mb-4">
                <label for="email" class="block text-sm font-medium text-gray-700">{{ __('messages.email') }}</label>
                <input type="email" name="email" id="email"
                    class="mt-1 block w-full border-perf-green rounded-[26px] shadow-sm" value="{{ $client->email }}"
                    required>
            </div>
            <div class="mb-4">
                <label for="phone" class="block text-sm font-medium text-gray-700">{{ __('messages.phone') }}</label>
                <input type="text" name="phone" id="phone"
                    class="mt-1 block w-full border-perf-green rounded-[26px] shadow-sm" value="{{ $client->phone }}">
            </div>
            <div class="mb-4">
                <label for="address"
                    class="block text-sm font-medium text-gray-700">{{ __('messages.address') }}</label>
                <input type="text" name="address" id="address"
                    class="mt-1 block w-full border-perf-green rounded-[26px] shadow-sm" value="{{ $client->address }}">
            </div>
            <div class="mb-4">
                <button type="submit"
